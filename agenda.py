...
class Email:
    ...
    def valida_email(email: str) -> bool:
        ...
        if not type(email) == str:
            raise TypeError('O email deve ser string')
        elif '_' in email or email.count('@') != 1 or email.replace('.','').isalnum():
            raise ValueError('O Erro no email alphanumerico')
        else:
            return True
...
class Contato:
    ...
    def nome(self, nome: str) -> None:
        ...
        if not isinstance(nome,str):
            raise TypeError('nome não é string')
        elif nome == '':
            raise CreateContactError('nome não pode ser vazio')
        else:
            self._nome = nome
    ...
    def apaga_telefone(self, tipo):
        ...
        """
        if tipo == 'principal':
            raise DeleteError('não deve permitir excluir o principal do telefone')
    ...
    def apaga_email(self, tipo):
        ...
        """
        if tipo == 'principal':
            raise DeleteError('não deve permitir excluir o principal do email')
    ...
    def lista_telefones(self) -> List[Tuple[str, Telefone]]:
        ...
        for tipo, telefone in self._telefones.items():
            lista_tel.append((tipo,telefone))
        return lista_tel
    ...
    def buscar(self, valor_busca: str):
        ...
        for a, b in self._telefones.items():
            if valor_busca in b.telefone:
                return True

        for a, b in self._emails.items():
            if valor_busca in b.email:
                return True
...
class Agenda:
    ...
    def ligar(self, valor_busca, tipo='principal') -> str:
        ...
        for contato in lista:
            for i in contato.lista_telefones():
                if tipo == i[0]:
                    return f'Ligando para {contato.nome}: {i[1]}'
    ...
    def apagar_contato(self, email_busca) -> str:
        ...
        for contato in lista:
            for s in contato.get_emails().values():
                if email_busca in s.email:
    ...
    def exportar_contatos(self, nome_arquivo: str) -> None:
        ...
        exporta_lista = []
        for contato in self.contatos:
            exporta_lista.append(contato.create_dump())
        
        files = open(nome_arquivo, 'w') 
        files.write(json.dumps(exporta_lista, default=dumper, indent=4, ensure_ascii=True)) 
